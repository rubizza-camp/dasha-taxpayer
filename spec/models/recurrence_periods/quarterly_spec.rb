...
RSpec.describe RecurrencePeriods::Quarterly, type: :model do
  ...
  it 'return valid date range' do
    expect(subject.next_period).to eq(Date.new(2018, 10, day_start)..Date.new(2018, 10, day_end))
  end

  it 'return date ranges in set interval' do
    periods = subject.periods(starts: Date.new(2019, 1, 1), until: Date.new(2020, 1, 1))

    expect(periods).to eq([Date.new(2019, 1, day_start)..Date.new(2019, 1, day_end),
