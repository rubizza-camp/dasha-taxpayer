...
Then(/^#{capture_model} should (?:be|have) (?:an? )?#{capture_predicate}$/) do |name, predicate|
  if model!(name).respond_to?("has_#{predicate.tr(' ', '_')}")
    expect(model!(name)).to send("have_#{predicate.tr(' ', '_')}")
  else
    expect(model!(name)).to send("be_#{predicate.tr(' ', '_')}")
  end
end
...
Then(/^#{capture_model} should not (?:be|have) (?:an? )?#{capture_predicate}$/) do |name, predicate|
  if model!(name).respond_to?("has_#{predicate.tr(' ', '_')}")
    expect(model!(name)).not_to send("have_#{predicate.tr(' ', '_')}")
  else
    expect(model!(name)).not_to send("be_#{predicate.tr(' ', '_')}")
  end
end
...
Then(/^#{capture_model}'s (\w+) (should(?: not)?) be #{capture_value}$/) do |name, attribute, expectation, expected|
  ...
  expectation   = expectation.gsub('should', 'to').tr(' ', '_')
